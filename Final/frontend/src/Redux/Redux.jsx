...
const Redux = createSlice({
  ...
  initialState,reducers:{
    ...
    updatequantity:(state,action)=>{
      const data=action.payload;
      // const exist=state.item.find((ite)=>ite.pid===data.pid);
      ...
      const value={
        ...
        pprice:data.pprice,
        pavailable:data.pavailable-data.quantity,
        ptype:data.ptype
      }
      console.log(value);
      state.quant.push(value);
    },
    removeupquant:(state,action)=>{
      state.quant=state.quant.filter((product)=>product.pid!==action.payload);
    },
    ...
    purchasepage:(state,action)=>{
      console.log(action.payload)
      state.purquant.push(action.payload);
    },
    removepur:(state,action)=>{
      state.purquant=state.purquant.filter((product)=> product.pid!== action.payload)
    },
  }
});
export const {SetAdmin,addsignup,addlogin,addproduct,removeproduct,updateproduct,setToUpdate,
  removeToUpdate,addToCart,removeFromCart,increasequantity,decreasequantity,addOrder,updatequantity,removeupquant,purchasepage,removepur}=Redux.actions;
...
export const getQuant=(state)=>state.adminInfo.quant;
export const getpur=(state)=>state.adminInfo.purquant;
