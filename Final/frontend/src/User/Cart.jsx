...
import { addOrder, decreasequantity, getCart, getOrder, getQuant, increasequantity, purchasepage, removeFromCart, updatequantity } from '../Redux/Redux'
...
product.forEach(prod=>{
  ...
  {
    dispatch(purchasepage(data));
    dispatch(updatequantity(data));
    navigate('/user/purchase');
  }
  else
  {
    console.log(`No stock available - ${prod.pname} only ${prod.pavailable} ${prod.ptype} available`);
    //  toast.error(`${prod.pname} only ${prod.pavailable} ${prod.ptype} available`);
        ...
        <div className="content d-flex-c">
         ...
         <table style={{width:'60vw'}} className='vptable'>
            ...
            <tbody style={{textAlign:'center'}}>
              ...
              <Button style={{backgroundColor:'green',color:'white',marginBottom:'20px'}} onClick={()=>myorder(prod)}>Checkout</Button>
         </div>
     </div>
    </div>
  )
